...
如果选择手动安装，你也可以选择不将`nmlc`的路径加入PATH中并在编译时手动指定编译器路径。
...
剩余的部分包含图像和其他代码。图像包含 realsprites（一般意义上的图像）与 recoloursprites（重着色图像）。每段代码都有其功能，如 A 代码块定义一辆载具与其属性，B 代码块定义载具使用的图像。整个 GRF 就是由一个个代码块和图像块构成的。

加载完成后，你就可以玩到 GRF 中的各种载具与建筑了。

### 制作身份证

如前文所说，OpenTTD在读取GRF的时候会读取GRF识别符，即ID。ID与其他项目必须放在NewGRF代码的最前面。grf块包含了这些项目。
